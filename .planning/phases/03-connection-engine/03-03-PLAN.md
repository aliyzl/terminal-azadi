...
must_haves:
  truths:
    ...
    - "azad connect <server> loads server store, finds server, fetches direct IP, starts engine, sets proxy, verifies proxy IP, compares direct vs proxy IP, and prints routing confirmation"
  ...
  artifacts:
    ...
    - path: "internal/engine/verify.go"
      provides: "IP verification through SOCKS5 proxy and direct IP fetch"
      exports: ["VerifyIP", "GetDirectIP"]
  ...
  key_links:
    ...
    - from: "internal/cli/connect.go"
      ...
      via: "Calls GetDirectIP before proxy, then VerifyIP after connecting, compares both"
      pattern: "engine\\.GetDirectIP|engine\\.VerifyIP"
    - from: "internal/cli/connect.go"
...
<task type="auto">
  ...
  <action>
    ...
    **Connection flow in RunE:**
    ...
    12. Fetch direct IP BEFORE verifying proxy: `directIP, directErr := engine.GetDirectIP()` (best-effort; if it fails, store empty string and continue)
    13. Verify connection: `proxyIP, err := engine.VerifyIP(cfg.Proxy.SOCKSPort)`
    14. If verify succeeds AND directIP was obtained: compare and print "Direct IP: %s -> Proxy IP: %s (routing confirmed)" if they differ, or print warning "Proxy IP matches direct IP -- routing may not be working" if they match
    15. If verify succeeds but directIP failed: print "Connected! Your IP: %s" with just the proxy IP
    16. If verify fails: print warning "Could not verify connection: %v" but stay connected
    17. Print status: "Status: %s | Server: %s | Press Ctrl+C to disconnect"
    18. Wait for context cancellation: `<-ctx.Done()`
    19. Print "Disconnecting..."
    20. Stop engine: `eng.Stop()`
    21. Unset system proxy: `sysproxy.UnsetSystemProxy(svc)`
    22. Remove .state.json
  ...
  <done>azad connect command orchestrates full flow: load config -> find server -> get direct IP -> start engine -> set proxy -> verify proxy IP -> compare direct vs proxy IP (routing confirmed) -> wait for signal -> stop engine -> unset proxy -> cleanup state. Build succeeds with all imports resolved.</done>
</task>
...
- azad connect fetches direct IP, then after proxy starts, fetches proxy IP and compares: prints "Direct IP: X -> Proxy IP: Y (routing confirmed)" when they differ
- azad connect finds server by name or uses last-used/first available
- Connect flow: get direct IP -> start engine -> write state -> set proxy -> verify proxy IP -> compare IPs -> wait -> cleanup
